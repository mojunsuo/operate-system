...
struct nfs_inode* nfs_alloc_inode(struct nfs_dentry * dentry) {
    ...
    int byte_cursor  = 0; //从索引位图中取空闲的指针
    int bit_cursor   = 0; //先一个字节一个字节地找，再一位一位地找
    int ino_cursor   = 0; //记录inode号
    int bno_cursor   = 0; //记录总的对应的数据块号
    int data_blk_cnt = 0; //记录inode的数据块使用
